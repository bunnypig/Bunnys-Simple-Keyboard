...
GLib.set_prgname("bunnyskeyboard")
...
key_mapping = {uinput.KEY_ESC: "Esc", uinput.KEY_1: "1", uinput.KEY_2: "2", uinput.KEY_3: "3", uinput.KEY_4: "4", uinput.KEY_5: "5", uinput.KEY_6: "6",
    ...
    uinput.KEY_LEFTBRACE: "Å", uinput.KEY_RIGHTBRACE: "¨", uinput.KEY_ENTER: "Enter", uinput.KEY_LEFTCTRL: "Ctrl_L", uinput.KEY_A: "A",
    uinput.KEY_S: "S", uinput.KEY_D: "D", uinput.KEY_F: "F", uinput.KEY_G: "G", uinput.KEY_H: "H", uinput.KEY_J: "J", uinput.KEY_K: "K",
    uinput.KEY_L: "L", uinput.KEY_SEMICOLON: "Ö", uinput.KEY_APOSTROPHE: "Ä", uinput.KEY_GRAVE: "§", uinput.KEY_LEFTSHIFT: "Shift_L",
...
class VirtualKeyboard(Gtk.Window):
    def __init__(self):
        ...
        self.set_wmclass("bunnyskeyboard", "bunnyskeyboard")
    ...
    def update_label(self, show_symbols):
        # Toggle labels for keys that have different unshifted/shifted symbols
        shift_map = {
            uinput.KEY_GRAVE: ("§", "½"),
            uinput.KEY_1: ("1", "!"),
            uinput.KEY_2: ("2", "\""),
        ...
        # self.key_buttons is populated in create_row()
        if not hasattr(self, "key_buttons"):
            return

        for key_event, pair in shift_map.items():
            btn = self.key_buttons.get(key_event)
            if not btn:
                continue
            btn.set_label(pair[1] if show_symbols else pair[0])
...
def install_kwin_rule():
    ...
    existing_rules = []
    if config.has_section("General") and config.has_option("General", "rules"):
        existing_rules = [
            rule.strip()
            for rule in config.get("General", "rules").split(",")
            if rule.strip()
        ]

    if group not in existing_rules:
        existing_rules.insert(0, group)

    commands = [
        ["kwriteconfig6", "--file", "kwinrulesrc", "--group", group, "--key", "Description", "bunnyskeyboard"],
        ["kwriteconfig6", "--file", "kwinrulesrc", "--group", group, "--key", "wmclass", "bunnyskeyboard bunnyskeyboard"],
        ["kwriteconfig6", "--file", "kwinrulesrc", "--group", group, "--key", "wmclassmatch", "1"],
        ["kwriteconfig6", "--file", "kwinrulesrc", "--group", group, "--key", "wmclasscomplete", "true"],
        ...
        ["kwriteconfig6", "--file", "kwinrulesrc", "--group", group, "--key", "above", "true"],
        ["kwriteconfig6", "--file", "kwinrulesrc", "--group", group, "--key", "aboverule", "2"],
        ["kwriteconfig6", "--file", "kwinrulesrc", "--group", "General", "--key", "rules", ",".join(existing_rules)],
        ["kwriteconfig6", "--file", "kwinrulesrc", "--group", "General", "--key", "count", str(len(existing_rules))],
    ]
